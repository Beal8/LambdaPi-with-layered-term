...
# 608 "src/unix/lwt_unix.cppo.ml"
  | O_KEEPEXEC

# 611 "src/unix/lwt_unix.cppo.ml"
...
# 1239 "src/unix/lwt_unix.cppo.ml"
  let fd = Unix.dup ?cloexec ch.fd in
  
# 1244 "src/unix/lwt_unix.cppo.ml"
  {
    fd = fd;
    state = Opened;
...
# 1263 "src/unix/lwt_unix.cppo.ml"
  Unix.dup2 ?cloexec ch1.fd ch2.fd;
  
# 1268 "src/unix/lwt_unix.cppo.ml"
...
# 1454 "src/unix/lwt_unix.cppo.ml"
  let (out_fd, in_fd) = Unix.pipe ?cloexec () in
  
# 1462 "src/unix/lwt_unix.cppo.ml"
  (mk_ch ~blocking:Sys.win32 out_fd, mk_ch ~blocking:Sys.win32 in_fd)

let pipe_in ?cloexec () =
   
# 1466 "src/unix/lwt_unix.cppo.ml"
   let (out_fd, in_fd) = Unix.pipe ?cloexec () in
  
# 1474 "src/unix/lwt_unix.cppo.ml"
  (mk_ch ~blocking:Sys.win32 out_fd, in_fd)

let pipe_out ?cloexec () =
  
# 1478 "src/unix/lwt_unix.cppo.ml"
  let (out_fd, in_fd) = Unix.pipe ?cloexec () in
  
# 1486 "src/unix/lwt_unix.cppo.ml"
...
# 1505 "src/unix/lwt_unix.cppo.ml"
    Lwt.return (Unix.symlink ?to_dir name1 name2)
  
# 1510 "src/unix/lwt_unix.cppo.ml"
  end else
...
# 1703 "src/unix/lwt_unix.cppo.ml"
  let s = Unix.socket ?cloexec dom typ proto in
  
# 1708 "src/unix/lwt_unix.cppo.ml"
...
# 1723 "src/unix/lwt_unix.cppo.ml"
...
# 1761 "src/unix/lwt_unix.cppo.ml"
    let (fd, addr) = Unix.accept ?cloexec ch_fd in
    
# 1766 "src/unix/lwt_unix.cppo.ml"
...
# 1898 "src/unix/lwt_unix.cppo.ml"
